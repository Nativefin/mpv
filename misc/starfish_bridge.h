...
// Call once from each decoder's create(), with whatever the demuxer/FFmpeg
// already told you. av_codec_id is a real enum AVCodecID (cast to int, so
// this header doesn't need <libavcodec/avcodec.h>) -- read it from
// codec->lav_codecpar->codec_id, not by re-deriving it from the codec name
// string. extradata may be NULL/0. profile is the AVCodecParameters profile
// field (AV_PROFILE_UNKNOWN if you don't have one).
//
// This may block briefly (bounded, ~500ms): if this is the video side and
// no audio has registered yet, it waits a short window for an audio decoder
// to show up (mpv creates video/audio decoders independently and in no
// guaranteed order, but Load() needs both codecs at once if the file has
// audio at all) before proceeding either way. Don't call this from a
// latency-sensitive path -- create() is fine, it's a one-time cost per
// session.
